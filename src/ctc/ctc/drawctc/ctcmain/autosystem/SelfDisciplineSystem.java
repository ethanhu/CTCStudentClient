...
import ctc.ctc.drawctc.station1.data.ParamFlag;
import ctc.ctc.drawctc.station1.data.RoadBasicInfo;
import ctc.ctc.drawctc.station1.data.RoadList;
import ctc.ctc.drawctc.station1.data.TrainStopRoad;
						...
						if (baseData.getAllStopRoadList() != null && baseData.getAllStopRoadList().size() != 0) {
							...
							for (int i = 0; i < len; i++) {
								...
								if (trainStopRoad.getStopLine().getStatus() == 2) { // 红色
									...
									if(trainStopRoad.getStopLine().getName().equalsIgnoreCase("X2JJ") || trainStopRoad.getStopLine().getName().equalsIgnoreCase("S2JJ")){
										SP.soundZBJC();									
									}
