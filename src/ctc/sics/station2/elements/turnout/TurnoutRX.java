package ctc.sics.station2.elements.turnout;
 ...
 * 
 */
public class TurnoutRX extends PolylineConnection {
	...
	public static TurnoutNameLabel nameLabel;

	/**
	 * 新建一个道岔
	 */
	public TurnoutRX(String name, int initX, int initY, String type) {

		this.name = name;
		this.type = type;
		nameLabel = new TurnoutNameLabel(name, initX - 5, initY + 10);

		mainLine.setForegroundColor(lineColor);
		mainLine.setStart(new Point(initX - h / 6, initY + h / 6));
		mainLine.setEnd(new Point(initX - h, initY + h));
		mainLine.setLineWidth(lineWidth);
		panel.add(mainLine);

		changeLine.setForegroundColor(backgroundColor);
		changeLine.setStart(new Point(initX, initY));
		changeLine.setEnd(new Point(initX - h / 6, initY + h / 6));
		changeLine.setLineWidth(lineWidth);
		panel.add(changeLine);

		trackLineChange.setForegroundColor(lineColor);
		trackLineChange.setStart(new Point(initX, initY));
		trackLineChange.setEnd(new Point(initX - h / 6, initY));
		trackLineChange.setLineWidth(lineWidth);
		panel.add(trackLineChange);

		trackLineMain.setForegroundColor(lineColor);
		trackLineMain.setStart(new Point(initX - h / 6, initY));
		trackLineMain.setEnd(new Point(initX - h, initY));
