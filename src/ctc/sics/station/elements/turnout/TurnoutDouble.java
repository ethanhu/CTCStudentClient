...
public class TurnoutDouble extends PolylineConnection {
	...
	public TurnoutDouble(String name, String nameLS, String nameRX, String nameLX, String nameRS, int initX, int initY, String type) {
		...
		this.name = name;		
		LSnameLabel = new TurnoutNameLabel(nameLS, initX - h / 2 - nameLS.length()*3 + 6, initY - h / 3 + 3);
		RSnameLabel = new TurnoutNameLabel(nameRS, initX + h / 2 - 6, initY - h / 3 + 3);
		LXnameLabel = new TurnoutNameLabel(nameLX, initX - h / 2 - nameLX.length()*3 + 6, initY + h / 3 - 9);
		RXnameLabel = new TurnoutNameLabel(nameRX, initX + h / 2 - 6, initY + h / 3 - 9);
...
*/		
			...
			this.changeLineXR.setForegroundColor(backgroundColor); //变化小段(下右
			break;
		case ParamFlag.turnout_dw_white: //定位白色
			this.status = -1;
			this.turnoutStatus = 1;
			this.mainLineL.setForegroundColor(ColorConstants.white); //主道岔(左倾斜);
