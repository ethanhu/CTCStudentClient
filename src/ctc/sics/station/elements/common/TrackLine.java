package ctc.sics.station.elements.common;
...
public class TrackLine extends PolylineConnection {
	...
	public TrackLine(String lineName, int X, int Y, int length, String type) {
		...
		int lineLen = lineName.length();
		if(lineLen < 4){
			lineLen = 4;
		}
		this.trainLabel.setSize(lineLen * 8 + 6, labHeight * 3 / 2);
	...
	// 股道线
	public TrackLine(Figure pan, String lineName, int X, int Y, int length, String type) {
		...
		this.trainLabel.setFont(font2);
		this.trainLabel.setForegroundColor(fontColor);				
		this.trainLabel.setSize(lineName.length() * 8 + 8, labHeight*3/2);
		if (type.equalsIgnoreCase("X")) { // 显示在上面
			this.trainLabel.setLocation(new Point(X + length / 2 - lineName.length() * 4 - 3, Y + 2));
		}else{
			this.trainLabel.setLocation(new Point(X + length / 2 - lineName.length() * 4 - 3, Y - labHeight*3/2 - 1));
		}		
		pan.add(this.trainLabel);
		...
		//股道名称
		new LineNameLabel(pan, lineName, X, Y, length, type);
	}
