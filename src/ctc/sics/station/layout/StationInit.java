...
import ctc.sics.station.autosystem.TrainRunSystem;
import ctc.sics.station.data.BaseParam;
import ctc.sics.station.drawstation.*;
import ctc.sics.station.autosystem.AutoSystem;
...
public class StationInit {
	...
	protected void createContents() {
		...
		selfDiscipline.TrainSelfDisciplineSystem(); //自动解锁
		
		TrainRunSystem.getInstance().processFA(); //自动进行车站内容闭塞分区解锁
		
		//启动列车计划信息自律机
		AutoSystem autoSystem = AutoSystem.getInstance();
		autoSystem.AutoSystemRun(); //自动执行任务
		autoSystem.initTrainRun();  //首站提前1分钟设置好车，末站自动1分钟后消失
	}
}
